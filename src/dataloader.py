...
class DatasetLoader(Dataset):
    def __init__(self, data, window_size):
        # Store the data and window size
        self.data = data
        self.window_size = window_size

        # Open the input file and store the file object
    ...
    def __getitem__(self, idx):
        # Read the lines from the file and return pairs (center, context, 5 negatives)
    ...
    @staticmethod
    def collate(batches):
        # Collate the batches into tensors
        all_u = []
        all_v = []
        all_neg_v = []
        for batch in batches:
            if len(batch) > 0:
                for u, v, neg_v in batch:
                    all_u.append(u)
                    all_v.append(v)
                    all_neg_v.extend(neg_v)
...
#     def __getitem__(self, idx):
#         # Read the lines from the file and return pairs (center, context, 5 negatives)
#         with open(self.data.inputFileName, encoding="ISO-8859-1") as input_file:
#             for _ in range(idx + 1):
